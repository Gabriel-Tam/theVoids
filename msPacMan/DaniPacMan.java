...
public final class DaniPacMan extends PacmanController {
    ...
    private int pacmanPowerTime(Game game) {
        ...
        for (GHOST ghost : GHOST.values()) {
            if (game.getGhostEdibleTime(ghost) > 0 || game.isGhostEdible(ghost)) {
                this.pacmanPowerTime = game.getGhostEdibleTime(ghost);
            } else {
                this.pacmanPowerTime = 0;
            }
        }
        System.out.println(pacmanPowerTime + " pacmanPowerTime");
        return pacmanPowerTime;
    }
